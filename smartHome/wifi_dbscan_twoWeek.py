...
def home_companywifi(userid,lines,l,collection):
    ...
    if homewifi:
        ...
        db.userprofiles_temp.update(userid,{"$set":{"timestamp":datetime.datetime.now(),\
...
if __name__ == '__main__':
    ...
    db = client['smartHome']
    collection = db.userprofiles_temp
    ...
    for each in alluser:
        ...
        userline = db.userprofiles_temp.find_one(userid)
        if userline  is None:
            db.userprofiles_temp.insert(userid)
        if  userline and homewifi and userline.get("homewifi") and userline.get( "longitude") :
            ...
            db.userprofiles_temp.update(userid,{"$set":{"timestamp":datetime.datetime.now(),"timepoints(300s)":timepoints300,\
